...
namespace ConsoleApp6
{
    class Program
    {
        ...
        {
            Squad firstSquad = new Squad(10);
            Squad secondSquad = new Squad(10);

            Console.WriteLine("Первый отряд: ");
            firstSquad.PrintSquad();
            Console.WriteLine();

            Console.WriteLine("Второй отряд: ");
            secondSquad.PrintSquad();
            Console.WriteLine();

            Console.WriteLine("Перевод бойцов с фамилией на Б в второй отряд: ");
            firstSquad.TransmitWarriorsToSquad(secondSquad);
            secondSquad.PrintSquad();
        }
    }
    ...
    class Squad
    {
        ...
        public void TransmitWarriorsToSquad(Squad squad)
        {
            List<Warrior> tempWarriors = _warriors.OrderBy(warrior=>warrior.LastName).Where(warrior =>warrior.LastName.StartsWith("Б")).ToList();
            squad._warriors= squad._warriors.Union(tempWarriors).ToList();
        }
